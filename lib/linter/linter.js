...
function parse(text, parser, providedParserOptions, filePath) {
    ...
    try {
        ...
        const parserSupportOptions = parseResult.parserSettings || { nodeIdentifier: "type", supportCodePathAnalysis: true, supportScope: true };
 ...
 */
function getScope(scopeManager, currentNode, nodeIdentifier) {

    // On Program node, get the outermost scope to avoid return Node.js special function scope or ES modules scope.
    const inner = currentNode[nodeIdentifier] !== "Program";

    for (let node = currentNode; node; node = node.parent) {
        const scope = scopeManager.acquire(node, inner);

        if (scope) {
            if (scope[nodeIdentifier] === "function-expression-name") {
...
function runRules(sourceCode, configuredRules, ruleMapper, parserOptions, parserName, settings, filename, disableFixes, cwd) {
    ...
    Traverser.traverse(sourceCode.ast, {
        ...
        nodeIdentifier: sourceCode.parserSupportOptions.nodeIdentifier
    });
    ...
    const sharedTraversalContext = Object.freeze(
        Object.assign(
            Object.create(BASE_TRAVERSAL_CONTEXT),
            {
                ...
                getScope: () => (sourceCode.parserSupportOptions.supportScope
                    ? getScope(sourceCode.scopeManager, currentNode, sourceCode.parserSupportOptions.nodeIdentifier)
                    : {}),
                getSourceCode: () => sourceCode,
                markVariableAsUsed: name => (sourceCode.parserSupportOptions.supportScope
                    ? markVariableAsUsed(sourceCode.scopeManager, currentNode, parserOptions, name)
                    : false),
                parserOptions,
    ...
    const eventGenerator = nodeQueue[0].node.type === "Program"
        ? new CodePathAnalyzer(new NodeEventGenerator(emitter, sourceCode.parserSupportOptions.nodeIdentifier))
        : new NodeEventGenerator(emitter, sourceCode.parserSupportOptions.nodeIdentifier);
